...
- [x] 1.20 PR-1 SQLite removal (design OD-5; OD-3 withdrawn) — strip what PR-1 shipped for the SQLite index, in one work unit: drop `better-sqlite3` and `@types/better-sqlite3` from `package.json` + lockfile; delete `src/boot/sqlite-probe.ts` and `test/boot/sqlite-probe.test.ts`; remove `SQLITE_FTS5_MISSING` from `AppErrorCode`/`ERROR_CODES` in `src/util/errors.ts` and its assertion in `test/util/errors.test.ts`; delete `docs/TROUBLESHOOTING.md` (its only content is the `#fts5` section) and any link to it; drop the better-sqlite3 mention from the closeable-registration doc comment in `test/helpers/create-test-vault.ts` (the helper stays — locks and watchers still use it) [boot-validation][index]. — done: commit `48baf9f`. `npm test` 183/183 (was 185/185 — the only delta is the 2 deleted probe tests), `npm run typecheck` clean, `npm run build` clean, zero native deps in `node_modules`; `rg -i 'sqlite|fts5'` over `src/`, `test/`, `docs/`, `package.json`, `package-lock.json` returns nothing (`docs/RFC.md` intentionally out of scope). Also reworded an unrelated "FTS5" example string in `test/rules/templates.test.ts` (arbitrary template-renderer fixture content, unrelated to the removed feature) to fully satisfy the grep gate.
  - **Verified before planning**: `validateBoot()` never calls `probeSqlite()`. The probe shipped standalone and its only planned call site was `src/mcp/server.ts` (task 2.16, not started), so there is **no FTS5 step to remove from `src/boot/validate-boot.ts`** — only the dead module, the error code, and the doc. Re-verified at apply time: still true, boot ordering unchanged.
  - **ESM-interop smoke test (design OD-3)**: no replacement test is needed, stated in the commit body rather than assumed. `gray-matter` is still a CJS package default-imported by `src/boot/validate-boot.ts` (and by `src/notes/parse.ts` from 2.6), so the existing `validate-boot` tests already prove NodeNext ESM→CJS default-import interop on Node 22 LTS.
  - **Spec amendment status**: already done ahead of this task, on this same branch (`cfb8c6d`, `5653f89`) — `specs/boot-validation/spec.md` no longer carries the FTS5 capability probe requirement, `specs/tool-catalog/spec.md`'s `find` requirement no longer names FTS5, and `openspec/config.yaml` no longer lists `better-sqlite3`. No spec/implementation contradiction remains for `sdd-verify`.
...
- [x] 2.2 `src/index/build.ts` — `buildIndex(vaultPath, rules)`: walk the vault's Markdown, parse each note with `gray-matter` (via `notes/parse.ts` derivation for id/title), populate the store. Asserts: nothing is written to disk (no index artifact anywhere in the vault, `git status` clean after a build) and two builds over the same vault yield identical query results (restart identity) [index]. — done (this commit): 7/7 (RED: module missing); walks only folders declared per note type in rules — `resolveNoteType`/`parseNoteAt` also exported and reused by 2.3's upsert.
- [x] 2.3 `src/index/upsert.ts` — incremental update without a rebuild: `upsertNote(store, note)` for the save path (saved/updated note immediately visible to `find` and backlink queries, no restart), and `reparseFiles(store, vaultPath, paths[])` for changes that arrive on disk — the post-pull case, where git names the changed files. Asserts: unchanged notes are not re-read; deletions drop the note and its link edges [index]. — done (this commit): 6/6 (RED: module missing). `upsertNote` re-reads the single named file from disk (deriving type via `resolveNoteType`) rather than taking a pre-built note — unifies the save path and `reparseFiles` on one code path; ENOENT on the named path is treated as a deletion.
- [x] 2.4 `src/index/queries.ts` — the only query surface: property filters (`type`/`status`/`spec_id`/`tags`/`owner`/date-range), free-text search over note bodies, combined filter+text (filters narrow first), and backlinks over the wikilink + `spec_id` graph. Deterministic result ordering (no relevance ranking — OD-5 states the tradeoff); no other module touches the store [index]. — done (this commit): 10/10 (RED: module missing). Ordering is a stable `path` sort; free text scans title + body, applied after property filters narrow the candidate set.
- [x] 2.5 `src/index/maps.ts` — deterministic regeneration of `index/` markdown maps (sorted, stable formatting) from the store — the committed, Obsidian-readable team surface and the input for P3's `chore(index)` commits [index]. — done (this commit): 5/5 (RED: module missing). One map per note type present in the store (`index/<type>.md`), entries sorted by path; exact markdown layout (bullet list, id + status) is this task's own design choice — neither the RFC nor the design doc specifies a literal format beyond "sorted, stable".
...
- [x] 2.8 `src/notes/save-pipeline.ts` — validate → render/merge → pull-before-write via injected `SyncPort` (null impl in P2) → write → `index.upsert` → `SyncPort.notifyWrite` → `{ path, id }` [tool-catalog]. — done (this commit): 5/5 (RED: module missing). Clarifications resolving ambiguity between this task's literal wording and design §5.3's tool-layer phrasing: (1) `saveNote` runs `validateNote` itself (defense in depth — usable standalone, whatever 2.13's `save` tool does upstream) and returns `ValidationIssue[]` verbatim on rejection via a discriminated `SaveNoteResult`; (2) validation runs against the *merged* (final) frontmatter, not the raw update patch — an update caller may send only the changed fields, and validating the patch in isolation would spuriously reject on fields it didn't repeat; (3) "render" for a create is direct frontmatter+body serialization (no `rules/templates.ts` involvement — template placeholder resolution is upstream, out of this PR's scope); "merge" for an update is a shallow frontmatter merge (new fields win) with the body replaced only if new content is supplied; (4) Linked Knowledge maintenance (2.7) is wired in here, hardcoding `"spec"` as the hub type name (matches RFC/design usage throughout — `NoteTypeDef` has no hub flag to derive it from).
