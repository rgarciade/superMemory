...
M1 scope: single-vault mode (`serve --vault`). The six MCP tools generated from `rules.md` (RFC §5): `find`, `read_with_context`, `save`, `changes_since`, `sync`, `status`. All mutations go through typed, validated operations.
