...
M1 scope: single-vault, `auto` sync mode plus the always-available manual `sync` trigger (tool and CLI). One sync engine shared by CLI and MCP server (RFC §6): triggers, pull-rebase flow, commit grammar, conflict ladder, secrets lint, and locking.
