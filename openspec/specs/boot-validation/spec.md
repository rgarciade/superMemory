# Boot Validation Specification

## Purpose

Vault path validation shared by `setup` and `serve` (RFC §3 boot-time validation): five checks, fail-fast with actionable messages. (Landed by change `add-m1-core`; M1 scope: single-vault mode.)
