...
Parses `.memory/rules.md` format v1 (RFC §4.2–§4.4): the team ontology as Markdown with typed fenced YAML blocks, plus `{{placeholder}}` template rendering by string interpolation only. (Landed by change `add-m1-core`; M1 scope: single-vault mode.)
