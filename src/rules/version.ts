 ...
 */
export function checkFormatVersion(declared: string | undefined): void {
  ...
  const required = parseMajor(declared);
  if (Number.isNaN(required)) {
    throw new AppError(
      "FORMAT_VERSION_UNSUPPORTED",
      `vault rules.md declares an invalid format_version "${declared}" — expected a semver like 1.0.`,
  ...
  if (required !== supported) {
    throw new AppError(
      "FORMAT_VERSION_UNSUPPORTED",
      unsupportedMessage(declared, required),
      {
